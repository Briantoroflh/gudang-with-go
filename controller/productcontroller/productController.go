...
import (
	"crud-menggunakan-golang/entities"
	"crud-menggunakan-golang/models/categorymodel"
	"crud-menggunakan-golang/models/productsmodel"
	"html/template"
	"net/http"
	"strconv"
	"time"
...
func Create(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		if ok := productsmodel.Add(product); !ok {
...
func Update(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		if ok := productsmodel.Edit(id, product); !ok {
			http.Redirect(w, r, r.Header.Get("Referer"), http.StatusTemporaryRedirect)
			return
		}
...
func Delete(w http.ResponseWriter, r *http.Request) {
	...
	if err := productsmodel.Remove(id); err != nil {
		panic(err)
	}
